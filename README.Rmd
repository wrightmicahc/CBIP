...
Calculate wildfire emissions under diffrent silvacultural treatments and biomass utilization scenarios as part of the California Biopower Impact Project (CBIP). The remote repository at [https://github.com/wrightmicahc/CBIP](https://github.com/wrightmicahc/CBIP) is for scripts only, no data are available.
...
The repository is in the Rstudio project format, so all scripts must be sourced relative to the main CBIP folder. Use a numeric tile number to estimate emissions for that tile.
...
|   +-- Tiles                   # Tile shapefiles, tiled inputs and output data
                                  sets.
...
|   +-- Other                   # misc. scripts
|   +-- emissions_model         # core scripts for the emissions model
...
This project was written in R using the Rstudio project framework. Occasionally, existing Python scripts were used, primarily through the rstudio interface with the reticulate package, though jupyter notebooks were also employed.
...
* The following R packages:
        - future.apply and its dependencies
        - data.table
...
The following code can be used to run the scenario_emissions function:
...
The model loads pre-processed spatial attributes that have been converted to tabular format with x-y indicator columns. The data all used the California (Teale) Albers projetion. The CRS is below:
...
total_char: char produced by scattered fuels in tons/acre including original fuelbed.
total_CH4: CH4 produced by scattered fuels in tons/acre including original fuelbed. 
total_CO: CO produced by scattered fuels in tons/acre including original fuelbed.
total_CO2: CO2 produced by scattered fuels in tons/acre including original fuelbed.
total_NH3: NH3 produced by scattered fuels in tons/acre including original fuelbed.
total_NOx: NOx produced by scattered fuels in tons/acre including original fuelbed.
total_PM10: PM10 produced by scattered fuels in tons/acre including original fuelbed.
total_PM2.5: PM2.5 produced by scattered fuels in tons/acre including original fuelbed.
total_SO2: SO2 produced by scattered fuels in tons/acre including original fuelbed.
total_VOC: VOC produced by scattered fuels in tons/acre including original fuelbed.
...
total_pile_NH3: NH3 from piled fuels in tons/acre.
...
total_duff_residue_NH3: NH3 produced by residue duff in tons/acre.
total_foliage_residue_NH3: NH3 produced by residue foliage in tons/acre.
total_fwd_residue_NH3: NH3 produced by residue fine woody debris (1-3") in tons/acre.
total_cwd_residue_NH3: NH3 produced by residue NH3arse woody debris (>3") in tons/acre.
