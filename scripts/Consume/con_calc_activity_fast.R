...
ccon_activity_fast <- function(dt, fm_type, days_since_rain){
        ...
        # make masks
        dt[,':='(mask_spring = pct_hun_hr <= 0.75, mask_trans = pct_hun_hr > 0.75 & pct_hun_hr < 0.85, mask_summer = pct_hun_hr >= 0.85, spring_ff = (pct_hun_hr - 0.75) / 0.1)]
        ...
        # fp <- ifelse(f > tlc, tlc, f), so if f < tlc, f
        #dt[flam_DRED >= QMD_100hr & flamg > total,flamg:=total]
        # All of which can be skipped, because flamg cannot exceed the total.
        ...
        #hundredhr <- list("flaming" = f,                            # flaming
        #                  "smoldering" = (tots - f) * (1.0 - rF),       # smoldering
        #                  "residual" = (tots - f) * rF,               # residual
        #                  "total" = tots)#
        #
        #flamgDRED <- list("flam_DRED" = flam_DRED, "flamg_portion" = flam_por)
        ...
        ###################################################
        csd <- c(1.0, 0.0, 0.0)
        dt[,':='(flamg_1 = (one_hr_sound * csd[1]), smoldg_1 = (one_hr_sound * csd[2]) ,resid_1 = (one_hr_sound * csd[3]), total_1 = (one_hr_sound * sum(csd)))]
        ...
        ###################################################
        # csd hasn't changed
        dt[,':='(flamg_10 = (ten_hr_sound * csd[1]), smoldg_10 = (ten_hr_sound * csd[2]) ,resid_10 = (ten_hr_sound * csd[3]), total_10 = (ten_hr_sound * sum(csd)))]
        ...
        dt[,':='(days_to_moist = 21.0 * ((duff_depth / 3.0)^1.18), days_to_dry = 57.0 * ((duff_depth / 3.0)^1.18))]
        ...
        dt[,':='(flaming_CH4 = flaming * ef_db$flaming[['CH4']], flaming_CO = flaming * ef_db$flaming[['CO']], flaming_CO2 = flaming * ef_db$flaming[['CO2']], flaming_NH3 = flaming * ef_db$flaming[['NH3']], flaming_NOx = flaming * ef_db$flaming[['NOx']], flaming_PM10 = flaming * ef_db$flaming[['PM10']], flaming_PM2.5 = flaming * ef_db$flaming[['PM2.5']], flaming_SO2 = flaming * ef_db$flaming[['SO2']], flaming_VOC = flaming * ef_db$flaming[['VOC']],
                 smoldering_CH4 = smoldering * ef_db$smoldering[['CH4']], smoldering_CO = smoldering * ef_db$smoldering[['CO']], smoldering_CO2 = smoldering * ef_db$smoldering[['CO2']], smoldering_NH3 = smoldering * ef_db$smoldering[['NH3']], smoldering_NOx = smoldering * ef_db$smoldering[['NOx']], smoldering_PM10 = smoldering * ef_db$smoldering[['PM10']], smoldering_PM2.5 = smoldering * ef_db$smoldering[['PM2.5']], smoldering_SO2 = smoldering * ef_db$smoldering[['SO2']], smoldering_VOC = smoldering * ef_db$smoldering[['VOC']],
                 residual_CH4 = residual * ef_db$residual[['CH4']], residual_CO = residual * ef_db$residual[['CO']], residual_CO2 = residual * ef_db$residual[['CO2']], residual_NH3 = residual * ef_db$residual[['NH3']], residual_NOx = residual * ef_db$residual[['NOx']], residual_PM10 = residual * ef_db$residual[['PM10']], residual_PM2.5 = residual * ef_db$residual[['PM2.5']], residual_SO2 = residual * ef_db$residual[['SO2']], residual_VOC = residual * ef_db$residual[['VOC']])]
        ...
        out_dt <- dt[,list(x, y, fuelbed_number, FCID2018, ID, Silvicultural_Treatment, Harvest_Type, Harvest_System, Burn_Type, Biomass_Collection, flaming_CH4, flaming_CO, flaming_CO2, flaming_NH3, flaming_NOx, 
...
#        ten_fsrt <- ccon_ten_act(ten_hr_sound)
        # 1,000 hr consumption
#        oneK_fsrt_snd <- ccon_oneK_act(oneK_hr_sound, QMDs, dred$diam_reduction, hun_hr_fsrt$flamgDRED$flam_DRED, HS = "H")
#        oneK_fsrt_rot <- ccon_oneK_act(oneK_hr_rotten, QMDs, dred$diam_reduction, hun_hr_fsrt$flamgDRED$flam_DRED, HS = "S")
        # 10,000 hr consumption
#        tenK_fsrt_snd <- ccon_tenK_act(tenK_hr_sound, QMDs, dred$diam_reduction, hun_hr_fsrt$flamgDRED$flam_DRED, HS = "H")
#        tenK_fsrt_rot <- ccon_tenK_act(tenK_hr_rotten, QMDs, dred$diam_reduction, hun_hr_fsrt$flamgDRED$flam_DRED, HS = "S")
        # consumption >10,000 hrs
#        tnkp_fsrt_snd <- ccon_tnkp_act(dred$adjfm_1000hr, hun_hr_fsrt$flamgDRED$flamg_portion, tnkp_hr_sound, HS = "H")
#        tnkp_fsrt_rot <- ccon_tnkp_act(dred$adjfm_1000hr, hun_hr_fsrt$flamgDRED$flamg_portion, tnkp_hr_rotten, HS = "S")
        # forest floor reduction
#        ffr <- ccon_ffr_activity(dred$diam_reduction, oneK_fsrt_snd, tenK_fsrt_snd, tnkp_fsrt_snd, days_since_rain, duff_upper_depth, duff_lower_depth, litter_depth, lichen_depth, moss_depth)
        # litter consumption
#        lit_fsrt <- ccon_forest_floor(litter_depth, litter_loading, ffr, c(0.90, 0.10, 0.0))
        # upper duff consumption
#        duff_fsrt <- ccon_forest_floor(duff_upper_depth, duff_upper_loading, ffr, c(0.10, 0.70, 0.20))
        # pile consumption
