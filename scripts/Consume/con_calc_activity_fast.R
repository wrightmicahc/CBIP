...
ccon_activity_fast <- function(dt, fm_type, days_since_rain, DRR){
        ...
        out_dt <- dt[,list(x, 
                           ...
                           one_hr_sound = (one_hr_sound - (flamg_1 + smoldg_1 + resid_1)) * one_hr_sound_prop, 
                           ten_hr_sound = (ten_hr_sound - (flamg_10 + smoldg_10 + resid_10)) * ten_hr_sound_prop,
                           hun_hr_sound = (hun_hr_sound - (flamg_100 + smoldg_100 + resid_100)) * hun_hr_sound_prop,
                           oneK_hr_sound = ((oneK_hr_sound - (flamg_OneK_snd + smoldg_OneK_snd + resid_OneK_snd)) * oneK_hr_sound_prop) + (pile_field - (flamg_pile_field + smoldg_pile_field + resid_pile_field)) + (pile_landing - (flamg_pile_landing + smoldg_pile_landing + resid_pile_landing)),
                           oneK_hr_rotten = oneK_hr_rotten - (flamg_OneK_rot + smoldg_OneK_rot + resid_OneK_rot),
                           tenK_hr_sound = (tenK_hr_sound - (flamg_tenK_snd + smoldg_tenK_snd + resid_tenK_snd)) * tenK_hr_sound_prop, 
                           tenK_hr_rotten = tenK_hr_rotten - (flamg_tenK_rot + smoldg_tenK_rot + resid_tenK_rot),
                           tnkp_hr_sound = (tnkp_hr_sound - (flamg_tnkp_snd + smoldg_tnkp_snd + resid_tnkp_snd)) * tnkp_hr_sound_prop,
