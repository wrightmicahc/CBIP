...
Make a raster of total emissions (not split into seperate phases). The following function creates a raster brick from for each treatment, with a layer for each emissions species. Save each treatment-level brick to a file.

```{r}
total_emissions <- dcast(consumption_df[c_phase == "total"],
                         x + y + Treatment + biomass_removed ~ e_spp,
                         value.var = "emissions")
